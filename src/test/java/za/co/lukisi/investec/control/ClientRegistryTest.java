...
public class ClientRegistryTest {
  ...
  @Test
  public void when_phone_number_is_requested_client_is_returned_with_phoneNumber(){

    Client clientByPhoneNumber = clientRegistry.getClientByPhoneNumber("0821234567");
    assertTrue(clientByPhoneNumber.getPhoneNumber().equals("0821234567"));
  }
  ...
  public void return_firstname_violation_when_null() {
    Client client = Client.builder().lastName("Luks").idNumber("840920").phoneNumber("10111").physicalAddress("That one").build();
  ...
  public void return_idnumber_violation_when_null() {
    Client client = Client.builder().lastName("Luks").firstName("Joe").phoneNumber("10111").physicalAddress("That one").build();
    Set<ConstraintViolation<Client>> violations = validator.validate(client);
    ConstraintViolation<Client> violation = violations.stream().findFirst().get();
  ...
  public void return_lastName_violation_when_null() {
    Client client = Client.builder().idNumber("840920").firstName("Joe").phoneNumber("10111").physicalAddress("That one").build();
    Set<ConstraintViolation<Client>> violations = validator.validate(client);
    ConstraintViolation<Client> violation = violations.stream().findFirst().get();
